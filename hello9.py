...
def change_theme():
    ...
    if current_theme == "default":
        # Check if the 'alt' theme already exists
        existing_themes = window.tk.call("ttk::style", "theme", "names")
        if "alt" not in existing_themes:
            # Create the 'alt' theme (dark mode)
            style.theme_create("alt", parent="default", settings={
        ...
        window.tk.call("ttk::style", "theme", "use", "alt")
        label.config(text="Current Theme: Dark")
    else:
        # Switch to the 'default' theme (light mode)
        window.tk.call("ttk::style", "theme", "use", "default")
        label.config(text="Current Theme: Light")
...
# Create a style object to modify the default theme
style = ttk.Style(window)


# Configure the window to use the default theme
window.tk.call("ttk::style", "theme", "default")

# Create a label to display the current theme
label = ttk.Label(window, text="Current Theme: Light")
label.pack(pady=20)

# Create a button to toggle the theme
button = ttk.Button(window, text="Toggle Theme", command=change_theme)
button.pack()
